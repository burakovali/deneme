...
def count_missing_values(didbName='didb', write_to_file=True):
    ...
    print(str(user_agent)+" of them are missing user agent|%"+str(100-100*(len(df) - user_agent)/len(df)))
    print(str(hostname)+" of them are missing hostname|%"+str(100-100*(len(df) - hostname)/len(df)))
    print(str(params)+" of them are missing params|%"+str(100-100*(len(df) - params)/len(df)))
    print(str(vendor)+" of them are missing vendor|%"+str(100-100*(len(df) - vendor)/len(df)))
    print(str(assoc_req_spatial_stream)+" of them are missing assoc request|%"+str(100-100*(len(df) - assoc_req_spatial_stream)/len(df)))
    print(str(assoc_req_vendors)+" of them are missing assoc request vendor|%"+str(100-100*(len(df) - assoc_req_vendors)/len(df)))
    print(str(wfa_device_name)+" of them are missing wfa device name|%"+str(100-100*(len(df) - wfa_device_name)/len(df)))
